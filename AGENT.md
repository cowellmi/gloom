...
- `hal.Rails` — board-level power rail control. Currently: generic `power.Controller` (`internal/power/power.go`). Each rail has a `threshold hal.RailState`; it is enabled when `Power(state)` is called with `state >= threshold`. Rails are configured entirely by board files in `initRails()` — they are not user-configurable via INI. Builds with `no_hypnos` tag skip rail control.
...
- `hal.Rails` — power rail control interface. `Power(state hal.RailState)` enables rails whose threshold <= state and disables the rest. Each rail carries its own stabilization delay; `Power` waits for the longest delay among newly-enabled rails before returning. Implementation lives in `internal/power/`. `power.Controller` tracks per-rail on/off state so repeated calls don't re-wait.
...
3. If the board has power rail control, add a build-tagged `power_<name>.go` in `cmd/gloom/` that provides `initRails() hal.Rails`. This function creates a `power.Controller` with per-rail pin, polarity, `hal.RailState` threshold, and stabilization delay, then returns it. The `no_hypnos` variant returns nil so no power code is compiled. Rail configuration is a compile-time board decision, not INI-configurable. Hypnos is the default for `feather_m0`; pass `-tags no_hypnos` to build without rail control.
